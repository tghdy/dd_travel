...
function showUserInf() {
    ...
    $.ajax({
        type: 'get',
        url: 'http://localhost:8080/dd_travel_war/user',
        data: {
            method: 'userInf',
        },
        dataType: 'json',
...
function exitUser() {
    $.ajax({
        ...
        success: function (json) {
            $('#login-inf').html(
                '<a href="http://localhost:8080/user/login.html" class="tt" style="font-size: 12px;position: relative;text-decoration: none;">登录</a>' +
                '<span style="position: relative;font-size: 13px;opacity: 0.3; margin: 0 5px;">|</span>' +
                '<a href="http://localhost:8080/user/register.html" class="tt" style="font-size: 12px;;position: relative;text-decoration: none;">免费注册</a>')
            // alert(json.msg);
        },
        error: function (data) {
            console.log(data.msg);
