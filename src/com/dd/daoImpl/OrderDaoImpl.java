...
public class OrderDaoImpl implements IOrderDao {
	...
	public int insertOrder(TravelOrder order) throws Exception {
		String sql = "insert into travel_order (order_no,order_state,user_id,travel_id,plan_seq,order_price,order_time)values (?,?,?,?,?,?,?)";
		JdbcUtils_DBCP.update(sql, new Object[]{order.getOrderNo(), order.getOrderState(), order.getUserId(), order.getTravelId(), order.getPlanSeq(), order.getOrderPrice(), order.getOrderTime()});
	...
	public List<Map<String, Object>> selectOrderList(Long userId, Integer lineType, Integer orderState) throws Exception {
		...
		sql.append("SELECT tot.*, tlt.travel_name, lp.start_time FROM travel_order tot, travel_line tlt, line_plan lp WHERE tot.travel_id = tlt.id ")
				.append(" AND lp.travel_id=tot.travel_id AND lp.seq=tot.plan_seq");
		if (userId != null) {
	...
	public Map<String, Object> selectOrderDetail(int id, int seq) throws Exception {
		String sql = "SELECT tot.*, tod.*, lp.* , tl.travel_name " +
				...
				"WHERE tot.id = tod.order_id AND tot.id = ? AND tot.travel_id = tl.id AND tl.id = lp.travel_id AND lp.seq = ?";
		//System.out.println(sql);
